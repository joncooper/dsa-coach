...
type PointEntry = { distance: number; x: number; y: number; point: number[] };

export function kClosestPointsLocal(points: number[][], k: number): number[][] {
  if (k <= 0) return [];
  const heap = new BinaryHeap<PointEntry>((a, b) => a.distance - b.distance || a.x - b.x || a.y - b.y);
  for (const point of points) {
    const [x, y] = point;
    heap.push({ distance: x * x + y * y, x, y, point });
  }

  const result: number[][] = [];
  const limit = Math.min(k, points.length);
  for (let count = 0; count < limit; count += 1) result.push(heap.pop()!.point);
  return result;
}
