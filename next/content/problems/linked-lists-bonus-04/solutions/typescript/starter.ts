...
export function reverseList(values: number[]): number[] {
  const head = buildList(values);
  throw new Error("TODO: reverse the linked list by changing next pointers, then return listToArray(newHead)");
}
