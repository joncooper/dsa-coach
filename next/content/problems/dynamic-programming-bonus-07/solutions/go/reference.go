...
func EditDistance(source string, target string) int {
	...
	for i := range dp { dp[i] = make([]int, len(target)+1); dp[i][0] = i }
	for j := 0; j <= len(target); j++ { dp[0][j] = j }
	for i := 1; i <= len(source); i++ { for j := 1; j <= len(target); j++ { if source[i-1] == target[j-1] { dp[i][j] = dp[i-1][j-1] } else { dp[i][j] = 1 + min3(dp[i-1][j], dp[i][j-1], dp[i-1][j-1]) } } }
