...
func MinPathSum(grid [][]int) int {
	...
	for row := 0; row < rows; row++ { for col := 0; col < cols; col++ { if row == 0 && col == 0 { dp[row][col] = grid[row][col] } else { up, left := 1<<60, 1<<60; if row > 0 { up = dp[row-1][col] }; if col > 0 { left = dp[row][col-1] }; dp[row][col] = grid[row][col] + min(up, left) } } }
