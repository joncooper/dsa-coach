...
type CloseEntry = { distance: number; value: number };

export function kClosestNumbers(nums: number[], target: number, k: number): number[] {
  const heap = new BinaryHeap<CloseEntry>((a, b) => a.distance - b.distance || a.value - b.value);
  for (const value of nums) heap.push({ distance: Math.abs(value - target), value });

  const chosen: number[] = [];
  const limit = Math.min(k, nums.length);
  for (let count = 0; count < limit; count += 1) chosen.push(heap.pop()!.value);
